...
Use this shorthand guide to move from a Strategic Idea to a Verified Line of Code for Slalom ETL-Pipeline.

## Phase 1: Strategic (The ADR)
1.  **Draft:** Create `docs/explanation/reports/adr/XXX_name.md`.
2.  **ID:** Assign `ADR-XXX`. Set `status: proposed`.
3.  **Epic:** `gh issue create --title "[ADR-XXX] Title" --label "Epic,Type: Architecture" --project "Slalom Project"`
...
1.  **Draft:** Create `docs/explanation/design/tdd/name.md`.
2.  **ID:** Assign `TDD-XXX`. Set `parent_adr: ADR-XXX`.
3.  **Feature:** `gh issue create --title "[TDD-XXX] Title" --label "feature,Type: Architecture" --project "Slalom Project"`
...
3.  **Audit:** Go to the [Project Board](https://github.com/users/mnaatjes/projects/4) and fill in the `ADR Link` and `TDD Link` for the new Task row.

## Phase 4: Implementation (The PR)
1.  **Branch:** `git checkout -b type/ISSUE-ID-slug` (e.g., `feat/101-fsspec-resolver`).
